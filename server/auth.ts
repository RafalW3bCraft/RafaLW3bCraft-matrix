...
export function requireAdmin(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  ...
  // SECURITY: Only designated admin email can access admin features
  ...
  // Strict validation: Must have admin role AND authorized email
  const isAuthorizedAdmin = user?.role === 'admin' && user?.email === AUTHORIZED_ADMIN_EMAIL;
  ...
  if (!isAuthorizedAdmin) {
    ...
    storage.createAuditLog({
      ...
      details: { 
        ...
        path: req.path,
        reason: user?.role === 'admin' ? 'unauthorized_email' : 'insufficient_role',
        expectedEmail: AUTHORIZED_ADMIN_EMAIL,
        actualEmail: user?.email || 'unknown'
      },
      ipAddress: req.ip,
      userAgent: req.get('User-Agent'),
      severity: 'warning'
...
export function isAdmin(req: AuthenticatedRequest): boolean {
  ...
  // Must have admin role AND authorized email
  return user?.role === 'admin' && user?.email === AUTHORIZED_ADMIN_EMAIL;
}